...
- 🔭 **当前状态**：积极寻找**日常实习**机会
- 💼 **实习意向**：后端开发实习生
- 💬 **技术兴趣**：Java、Spring、MySQL、Redis、JUC、分布式系统
...
[![GitHub](https://img.shields.io/badge/GitHub-代码仓库-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/0214ZhuangLi)
...
**主要工作：**
- **DevOps工具链建设**：使用Nexus搭建私有制品仓库，集成Jenkins实现CI/CD自动化流水线，为团队提供完整的持续集成和部署解决方案，显著提升了代码发布效率和质量管控
- **项目模板管理工具开发**：独立开发基于JGit的项目模板管理工具，实现从Gitee自动拉取启动模板并发布至私有仓库的全流程自动化，有效提升了新项目初始化的开发效率
- **跨平台部署适配**：参与核心业务系统在华为麒麟v10 ARM架构的跨平台部署工作，深入研究并解决了架构兼容性问题，成功协助公司获得华为麒麟认证，为产品在国产化环境的推广奠定了基础

**技术收获：**
- 深入掌握了DevOps工具链的设计和实施，理解了CI/CD在企业级项目中的重要作用
- 熟练使用JGit进行Git操作的编程实现，提升了版本控制系统的底层理解
- 积累了跨平台部署和架构兼容性问题的解决经验，了解了ARM架构的技术特点

**个人成长：**
虽然这次实习中我接触到的后端开发知识有限，但我更加深刻地理解了工程化思维，提升了在真实项目中解决复杂问题的能力。同时，我也初步了解了公司级项目从设计、开发到上线的完整流程，这为我今后的技术成长打下了坚实的基础。这次经历让我对软件开发的全生命周期有了更全面的认识。
...
**研究背景：** 本研究是在导师指导下完成的小论文项目，针对知识图谱补全(KGC)任务中现有方法对图结构信息利用不充分的问题，提出了基于图神经网络的渐进式知识图谱补全模型，旨在更好地融合语义信息和结构信息。

**研究内容：**
- **图神经网络融合**：将图神经网络(GNN)引入知识图谱补全任务，通过消息传递机制更好地捕获实体间的结构关系，提升了模型对复杂图结构的表示能力
- **渐进式训练框架**：设计Progressive训练框架，采用两阶段策略逐步引入GNN信息——先稳定语义信息学习，再逐步融入结构信息，避免了训练不稳定问题
- **实验设计**：在公开数据集WN18RR和FB15k-237上与多个现有的知识图谱补全模型进行对比实验，使用Hit@1、Hit@10和MRR等标准评估指标验证模型效果
- **技术实现**：基于TensorFlow框架实现模型架构，使用Python进行数据预处理和图算法实现，构建了完整的训练和评估流水线

**主要贡献：**
- 在WN18RR和FB15k-237数据集上取得了相比现有方法更好的实验效果，验证了渐进式GNN框架的有效性
- 提出了语义信息与结构信息分阶段融合的新思路，为知识图谱补全任务提供了新的解决方案
- 完成了完整的实验代码实现，具备良好的可复现性和扩展性
...
**研究收获：**
通过这个研究项目，我不仅深入理解了图神经网络和知识表示学习的核心原理，更重要的是培养了系统性的工程能力。在实现复杂模型架构的过程中，我学会了如何设计清晰的代码结构、处理大规模数据集、优化训练流程，这些经验直接转化为了我的后端开发能力。特别是在构建实验管理系统时，我掌握了配置管理、日志记录、性能监控等软件工程实践，理解了如何构建稳定可靠的数据处理流水线。同时，多模型对比实验让我具备了很强的性能调优和问题定位能力，这对后端系统优化同样适用。
